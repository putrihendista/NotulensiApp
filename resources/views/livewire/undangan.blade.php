<div class="flex">
    <!-- Sidebar -->
    <div class="w-auto p-4 bg-blueLight">
        @livewire('sidebar')
    </div>

    <!-- Konten Utama -->
    <div class="w-3/4 max-w-4xl mx-auto p-6 bg-white border border-gray-300 rounded-lg shadow-md mt-4">
        <h2 class="font-semibold">Surat Undangan Rapat Asistensi Sakip 2024</h2>
        <div class="mt-6">
            <p class="text-gray-700 mb-4">
                Surat undangan rapat asistensi Sistem Akuntabilitas Kinerja Instansi Pemerintah (SAKIP) 2024 yang akan 
                diselenggarakan secara luring pada 15 September 2024 di Gedung B302 oleh semua staff yang telah diberikan 
                undangan perihal tersebut.... 
                <a href="#" class="text-blue-500 font-semibold">Baca Selengkapnya</a>
            </p>    
        </div>  
        <div class="grid justify-items-end">
            <button class="bg-blueLight text-white py-1 px-3 rounded-md font-semibold mt-8 mb-12 flex items-center justify-between">Download</button>
        </div>
        <div>
            <a href="/undanganlainnya" class="text-black font-semibold">Lihat Undangan Lainnya</a>
        </div>
    </div>
</div>
